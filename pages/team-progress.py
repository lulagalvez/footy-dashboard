...
dash.register_page(__name__, path='/team-progress', name="Progreso de Equipos")
...
teams = pd.unique(matches_df[['home_team_name', 'away_team_name']].values.ravel('K'))
team_progress = {team: pd.DataFrame(columns=['timestamp', 'points']) for team in teams}
...
for index, row in matches_df.iterrows():
    ...
    if team_progress[home_team].empty:
        new_row = pd.DataFrame({'timestamp': [timestamp], 'points': [home_points]})
        team_progress[home_team] = pd.concat([team_progress[home_team], new_row], ignore_index=True)
    else:
        last_points = team_progress[home_team].iloc[-1]['points']
        new_row = pd.DataFrame({'timestamp': [timestamp], 'points': [last_points + home_points]})
        team_progress[home_team] = pd.concat([team_progress[home_team], new_row], ignore_index=True)
    
    if team_progress[away_team].empty:
        new_row = pd.DataFrame({'timestamp': [timestamp], 'points': [away_points]})
        team_progress[away_team] = pd.concat([team_progress[away_team], new_row], ignore_index=True)
    else:
        last_points = team_progress[away_team].iloc[-1]['points']
        new_row = pd.DataFrame({'timestamp': [timestamp], 'points': [last_points + away_points]})
        team_progress[away_team] = pd.concat([team_progress[away_team], new_row], ignore_index=True)
...
all_teams_progress['timestamp'] = pd.to_datetime(all_teams_progress['timestamp'])
...
fig = px.line(all_teams_progress, x='timestamp', y='points', color='team',
              labels={'timestamp': 'Fecha', 'points': 'Puntos', 'team': 'Equipo'},
              title='Progreso de los Equipos en la Temporada')

# Update the color of each team
fig.for_each_trace(lambda trace: trace.update(line=dict(color=team_colors.get(trace.name))))

####################### PAGE LAYOUT #############################
layout = html.Div(children=[
    html.H2("Progreso de los Equipos en la Temporada"),
